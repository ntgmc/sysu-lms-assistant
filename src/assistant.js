...
import {
    fillForumReply as fillForumReplyInForm,
    findForumSubmitButton as findMoodleForumSubmitButton,
    findForumTextarea as findMoodleForumTextarea,
    getForumActivityId as readForumActivityId,
    getForumCompletionRequirement as readForumCompletionRequirement,
    getForumDiscussionId as readForumDiscussionId,
    getNormalizedPageUrl as normalizePageUrl,
    getReplyTargets as readReplyTargets
} from './forum-dom.js';
import {
    createNavigationScheduler,
    getNavigationCancellation
} from './navigation.js';
import { classifyPage, getPageKey, PAGE_TYPES } from './page.js';
import {
    chooseRandomItem,
    createSettingsStore,
    parseForumReplyTemplates
} from './settings.js';
import { captureNativeTimers, installTimerAcceleration } from './timers.js';
import { findHdOption, getCompletionState } from './video.js';
    ...
    function createLmsController(ui) {
        ...
        function checkPage() {
            try {
                ...
                attachVideo(video);
                ui.setPageType(pageType === PAGE_TYPES.VIDEO
                    ? '视频课程'
                    : (pageType === PAGE_TYPES.RESOURCE ? '资源停留页' : '普通课程页'));
        ...
        function getForumCompletionRequirement() {
            return readForumCompletionRequirement(document);
        }

        function getForumActivityId() {
            return readForumActivityId(window.location);
        }

        function getForumDiscussionId() {
            return readForumDiscussionId(document, window.location);
        }

        function getNormalizedPageUrl() {
            return normalizePageUrl(window.location);
        }
        ...
        function getReplyTargets() {
            return readReplyTargets(document, window.location);
        }
        ...
        function findForumTextarea(form) {
            return findMoodleForumTextarea(form);
        }

        function findForumSubmitButton(form) {
            return findMoodleForumSubmitButton(form);
        }

        function fillForumReply(textarea, contentEditable, replyText) {
            fillForumReplyInForm(textarea, contentEditable, replyText);
        }
        ...
        function scheduleNavigation(kind, nextLink, delay, message, toastKey) {
            ...
            const scheduled = navigation.schedule({
                ...
                link: nextLink,
                delay,
                isCurrent: () => settings.assistantEnabled
                    && (kind === 'forum-skip'
                        ? settings.skipForum && !settings.autoCompleteForum
                        : settings.autoNext),
